...
def getValues(num, stack, args=None, compute=None):
    if args is not None:
        return args[:num]
    # V1 mode
    values = []
...
def callFunction(compute, function, stack, args=None):
    match function:
        case "pow":
            values = getValues(2, stack, args=args, compute=compute)
        ...
        case "sqrt":
            value = getValues(1, stack, args=args, compute=compute)[0]["value"]
            # 文字列ならエラー
        ...
        case "int":
            value = getValues(1, stack, args=args, compute=compute)[0]
        ...
        case "replace":  # replace(string, old, new)
            values = getValues(3, stack, args=args, compute=compute)
            new = values[0]["value"]
            old = values[1]["value"]
        ...
        case "upper":  # upper(string)
            string = getValues(1, stack, args=args, compute=compute)[0]
            return True, {
        ...
        case "not":  # not(condition)
            condition = getValues(1, stack, args=args, compute=compute)[0]["value"]
            if condition == 1:
        ...
        case "random_string":  # randomstring(length)
            length = getValues(1, stack, args=args, compute=compute)[0]["value"]
            return True, {
                "type": TOKENTYPE.STRING,
                "value": "".join(
                    [
        ...
        case "value":
            ...
            string = getValues(1, stack, args=args, compute=compute)[0]["value"]
        ...
        case "choice_index":
            variables = getValues(3, stack, args=args, compute=compute)
        ...
        case "choice":
            ...
            variable = getValues(1, stack, args=args, compute=compute)[0]["value"]
            if isinstance(variable, str) is False:
                compute.setTokenError(
